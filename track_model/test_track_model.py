...
def make_data_dict(data_dir):
    ...
    for i, video in enumerate(videos):
        ...
        gtbox_file = ''.join([data_dir, video_name, '/groundtruth_rect.txt'])
        d[video_nr] = (video_name, video_file, gtbox_file)
    return d

def load_video_metadata(video_name, gtbox_file):
    s = open(gtbox_file).read()
    s = s.replace(' ', ',')
    s = s.replace('\t', ',')
    gtboxes = np.loadtxt(StringIO.StringIO(s), delimiter=',').reshape((-1, 4))
...
def inference(config):
    ...
    ################################################################################
    data_dict = make_data_dict(config.data_dir)
    total = len(data_dict)
    ...
    for video_nr in range(total):
        ...
        start_frame_id, gtboxes = load_video_metadata(video_name, gtbox_file)
        num_frames = gtboxes.shape[0]
