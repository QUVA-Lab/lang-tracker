...
def make_data_dict(test_file, data_dir):
    ...
    for i, video in enumerate(videos):
        ...
        start_frame_id = frame_starts[i]
        end_frame_id = frame_ends[i]
        d[video_nr] = (video_name, video_file, gt_box_file, start_frame_id, end_frame_id)
    return d

def load_video_metadata(video_name, gt_box_file):
    try:
        gt_boxes = np.loadtxt(gt_box_file, delimiter=',').reshape((-1, 5))
    except ValueError:
        gt_boxes = np.loadtxt(gt_box_file).reshape((-1, 5))
...
def inference(config):
    ...
    ################################################################################
    data_dict = make_data_dict(config.test_file, config.data_dir)
    total = len(data_dict)
    print('In total %d videos to evaluate.'%total)
    dummy_label = np.zeros((config.N, 1))

    for video_nr in range(total):
        video_name, video_file, gt_box_file, start_frame_id, end_frame_id = data_dict[video_nr]
        
        gt_boxes = load_video_metadata(video_name, gt_box_file)
        num_frames = gt_boxes.shape[0]
        #print('%d %d\n'%(end_frame_id-start_frame_id+1, num_frames))
        assert (end_frame_id-start_frame_id+1) == num_frames
        print('Start processing video %s (%d frames).'%(video_name,num_frames))
        
        query_ids = range(2)
        for q in query_ids:
            pred_box_file = ''.join([config.pred_dir, video_name, '_query_', str(q), '_prediction_rect.txt'])
            try:
                pred_boxes = np.loadtxt(pred_box_file, delimiter=',').reshape((-1, 4))
            except ValueError:
                pred_boxes = np.loadtxt(pred_box_file).reshape((-1, 4))
            ...
            qimg = skimage.io.imread('%s/img/%06d.jpg' % (video_file, start_frame_id))
            ...
            for ii in tqdm(range(start_frame_id+1, num_frames+start_frame_id)):
                img = skimage.io.imread('%s/img/%06d.jpg' % (video_file, ii))
                if img.ndim == 2:
            ...
            filename = config.result_dir+video_name+'_query_'+str(q)+'_'+config.signature+'.txt'
            fp = open(filename, 'w')
...
if __name__ == '__main__':
    config = test_ilsvrc_pred_config.Config()
    inference(config)
